...
function renderPricingPage(): void {
  const app = document.getElementById('app')!;
  const currentPlan = getUserData()?.plan || 'free';
  
  app.innerHTML = `
    <div class="pricing-container">
      <div class="pricing-header">
        <button class="btn btn-ghost" id="btn-pricing-back" style="margin-bottom: 24px;">← Back to Copilot</button>
        <h1>Choose Your Plan</h1>
        <p>Unlock the full potential of TwinMind AI for your meetings.</p>
      </div>
      ...
      <div class="pricing-cards">
        ...
        <div class="pricing-card ${currentPlan === 'free' ? 'current' : ''}">
          ...
          <ul class="plan-features">
            <li>15-min meeting limit</li>
            <li>Basic AI Suggestions</li>
            <li>Llama-3 8B Access</li>
            <li>LocalStorage Persistence</li>
          </ul>
          <button class="btn ${currentPlan === 'free' ? 'btn-ghost disabled' : 'btn-ghost'}" ${currentPlan === 'free' ? 'disabled' : ''}>
            ${currentPlan === 'free' ? 'Current Plan' : 'Downgrade'}
          </button>
        </div>
        
        <!-- Pro Plan -->
        <div class="pricing-card popular ${currentPlan === 'pro' ? 'current' : ''}">
          <div class="plan-name">Pro</div>
          <div class="plan-price">$19<span>/mo</span></div>
          <ul class="plan-features">
            <li>Unlimited meeting length</li>
            <li>Advanced Llama-3 70B</li>
            <li>Detailed Meeting Reports</li>
            <li>Smart Action Items</li>
            <li>Priority AI Processing</li>
          </ul>
          <button class="btn btn-primary" id="btn-buy-pro" ${currentPlan === 'pro' ? 'disabled' : ''}>
        ...
        <div class="pricing-card ${currentPlan === 'enterprise' ? 'current' : ''}">
          ...
          <ul class="plan-features">
            ...
            <li>Dedicated API Endpoint</li>
            <li>SSO & Advanced Security</li>
          </ul>
          <button class="btn btn-ghost" id="btn-buy-enterprise" ${currentPlan === 'enterprise' ? 'disabled' : ''}>
  ...
  document.getElementById('btn-pricing-back')!.addEventListener('click', renderApp);
  
  const buyPro = document.getElementById('btn-buy-pro');
  if (buyPro) buyPro.addEventListener('click', () => handleUpgrade('pro'));
  
  const buyEnterprise = document.getElementById('btn-buy-enterprise');
  if (buyEnterprise) buyEnterprise.addEventListener('click', () => handleUpgrade('enterprise'));
}

async function handleUpgrade(plan: UserPlan): Promise<void> {
  // Simulate payment processing
  showToast('Processing payment...', 'info');
  await new Promise(resolve => setTimeout(resolve, 1500));
  
  updateUserPlan(plan);
  showToast(`Successfully upgraded to ${plan.toUpperCase()}!`, 'success');
  renderApp();
}
